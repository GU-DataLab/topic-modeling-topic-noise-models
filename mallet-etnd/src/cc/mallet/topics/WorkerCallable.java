...
public class WorkerCallable implements Callable<Integer> {
    ...
    int[][] docEmb; // indexed by <document index, sequence index>
	int[] noiseDistribution; // indexed by <word index>
    double skew;

    HashMap<String, List<String>> optCloseWords;
    HashMap<String, List<String>> closeWords;
    int iteration = 0;
    ...
    public WorkerCallable (int numTopics,
        ...
        this.docEmb = docEmb;
        this.skew = skew;
        this.optCloseWords = optCloseWords;
    ...
    public int[][] getDocEmb() { return docEmb; }
        ...
        for (int doc = startDoc; doc < data.size() && doc < startDoc + numDocs; doc++) {
            ...
            for (int position = 0; position < tokens.size(); position++) {
                ...
                if (docNoise[doc][position] == 1) {
                    ...
                    if (docEmb[doc][position] > 0) {
                        Object wordObj = alphabet.lookupObject(type);
                        String word = wordObj.toString();
                        if (closeWords.get(word) != null) {
                            List<String> embeddedWords = closeWords.get(word);
                            for (int embPos = 0; embPos < embeddedWords.size(); embPos++) {
                                int embType = alphabet.lookupIndex(embeddedWords.get(embPos), false);
                                if (embType > -1) {
                                    noiseDistribution[embType]++;
                                }
                            }
                        }
                    }
                    ...
                    tokensPerTopic[topic] += 1 + docEmb[doc][position];
	...
	private boolean noiseRoll(int word, int topic_count) {
		...
		assert (roll <= 1 && 0 <= roll) : "BetaDistribution is spitting out bad rolls: " + roll + ". Noise: " + noise_level + ", Signal: " + signal_level;
        return roll < 0.5;
    }
                    ...
                    if (closeWords.get(word) != null) {
                        ...
                        for (int embPos = 0; embPos < embeddedWords.size(); embPos++) {
                            ...
                            if (embType > -1) {
                                ...
                                int currentEmbTopic;
                                int currentEmbValue;
                                int[] currentEmbTypeTopicCounts = typeTopicCounts[type];
                                boolean embDecremented = false;
                                while (embIndex < currentEmbTypeTopicCounts.length &&
                                        currentEmbTypeTopicCounts[embIndex] > 0) {
                                    currentEmbTopic = currentEmbTypeTopicCounts[embIndex] & topicMask; // get topic bits
                                    currentEmbValue = currentEmbTypeTopicCounts[embIndex] >> topicBits; // get topic-term frequency bits

                                    if (!embDecremented &&
                                            currentEmbTopic == oldTopic) {
                                        ...
                                        removedEmbWords++;
                                        docEmb[doc][position]--;
                                        // possible here to decrement before the embedded word has a positive count, so changed == to <=
                                        if (currentEmbValue <= 0) {
                                            if (currentEmbValue < 0) {
                                                currentEmbValue = 0;
                                                removedEmbWords--;
                                                docEmb[doc][position]++;
                                            ...
                                            currentEmbTypeTopicCounts[embIndex] =
                                                    (currentEmbValue << topicBits) + oldTopic;
                                        }

                                        // Shift the reduced value to the right, if necessary.

                                        @Var
                                        int subIndex = embIndex;
                                        while (subIndex < currentEmbTypeTopicCounts.length - 1 &&
                                                currentEmbTypeTopicCounts[subIndex] < currentEmbTypeTopicCounts[subIndex + 1]) {
                                            int temp = currentEmbTypeTopicCounts[subIndex];
                                            currentEmbTypeTopicCounts[subIndex] = currentEmbTypeTopicCounts[subIndex + 1];
                                            currentEmbTypeTopicCounts[subIndex + 1] = temp;
...
//                 System.err.println("Embedded Words removedEmbWords: " + removedEmbWords);
                assert (tokensPerTopic[oldTopic] >= 0) : "old Topic " + oldTopic + " below 0 prior to decrement: " + tokensPerTopic[oldTopic];
                tokensPerTopic[oldTopic] -= (1 + removedEmbWords);
                assert (tokensPerTopic[oldTopic] >= 0) : "old Topic " + oldTopic + " below 0: " + tokensPerTopic[oldTopic];
...
//                localTopicCounts[oldTopic]++;
                ...
                if (docEmb[doc][position] > 0) {
                    ...
                    if (closeWords.get(word) != null) {
                        ...
                        for (int embPos = 0; embPos < embeddedWords.size(); embPos++) {
                            int embType = alphabet.lookupIndex(embeddedWords.get(embPos), false);
                            if (embType > -1 && noiseDistribution[embType] > 0) {
                                noiseDistribution[embType]--;
                                docEmb[doc][position]--;
                            }
                        }
...
//                     assert (currentTopic-1 >= 0) : "Current Topic " below 0: "currentTopic";
                    score =
...
//                        noiseDistribution[type]++;
            ...
            if (docNoise[doc][position] == 0) {
                Object wordObj = alphabet.lookupObject(type);
                String word = wordObj.toString();
                if (optCloseWords.get(word) != null) {
                    List<String> embeddedWords = optCloseWords.get(word);
                    for (int embPos = 0; embPos < embeddedWords.size(); embPos++) {
                        int embType = alphabet.lookupIndex(embeddedWords.get(embPos), false);
                        if (embType > -1) {
                            int embIndex = 0;
                            ...
                            int currentEmbValue;
                            int[] currentEmbTypeTopicCounts = typeTopicCounts[type];
                            boolean embIncremented = false;
                            while (embIndex < currentEmbTypeTopicCounts.length &&
                                    currentEmbTypeTopicCounts[embIndex] > 0) {
                                currentEmbTopic = currentEmbTypeTopicCounts[embIndex] & topicMask; // get topic bits
                                currentEmbValue = currentEmbTypeTopicCounts[embIndex] >> topicBits;
                                if (!embIncremented &&
                                currentEmbTopic == newTopic) {
                                    currentEmbTypeTopicCounts[embIndex] = ((currentEmbValue + 1) << topicBits) + newTopic;
                                    embIncremented = true;
                                    addedEmbWords++;
                                    docEmb[doc][position]++;
                                } else {
                            ...
                            while (embIndex > 0 &&
                                    currentEmbTypeTopicCounts[embIndex] > currentEmbTypeTopicCounts[embIndex - 1]) {
                                int temp = currentEmbTypeTopicCounts[embIndex];
                                currentEmbTypeTopicCounts[embIndex] = currentEmbTypeTopicCounts[embIndex - 1];
                                currentEmbTypeTopicCounts[embIndex - 1] = temp;

                                embIndex--;
                            }
                        }
                    }
//                     System.err.println("Embedded Words added: " + addedEmbWords + docEmb[doc][position]);
                }
                ...
                tokensPerTopic[newTopic] += 1 + docEmb[doc][position];
