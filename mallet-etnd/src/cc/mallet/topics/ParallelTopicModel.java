...
public class ParallelTopicModel implements Serializable {
    ...
    public int[][] docNoise; // indexed by <document index, sequence index>
	public int[] noiseDistribution; // indexed by <feature index>

	//for embedding space
	public int[][] docEmb; // indexed by <document index, sequence index>

    public int tau; // number of iterations between embedding space reoptimization
    public HashMap<String, List<String>> closeWords;
    public HashMap<String, List<String>> optCloseWords = new HashMap<String, List<String>>();
    ...
    public ParallelTopicModel (int numberOfTopics, HashMap<String, List<String>> closeWords) {
        this (numberOfTopics, numberOfTopics, DEFAULT_BETA, 0.0, 200, closeWords);
    }

    public ParallelTopicModel (int numberOfTopics, double skew, HashMap<String, List<String>> closeWords)
	{
		this (numberOfTopics, 50.0, 0.01, skew, 200, closeWords);
	}

	public ParallelTopicModel (int numberOfTopics, double alphaSum, double beta, double skew, int tau, HashMap<String, List<String>> closeWords) {
        this (newLabelAlphabet (numberOfTopics), alphaSum, beta, skew, tau, closeWords);
    }

    public ParallelTopicModel (int numberOfTopics, double alphaSum, double beta, HashMap<String, List<String>> closeWords) {
        this (newLabelAlphabet (numberOfTopics), alphaSum, beta, 0.0, 200, closeWords);
    }
    ...
    public ParallelTopicModel (LabelAlphabet topicAlphabet, double alphaSum, double beta, double skew, int tau, HashMap<String, List<String>> closeWords) {
        ...
        logger.info("Mallet eNFT: " + numTopics + " topics, " + topicBits + " topic bits, " +
    ...
    public void buildInitialTypeTopicCounts () {
        ...
        tokensPerTopic = new int[numTopics];
        noiseDistribution = new int[numTypes];
        docNoise = new int[data.size()][];
        docEmb = new int[data.size()][];
        ...
        for (int doc = 0; doc < data.size(); doc++) {
            ...
            FeatureSequence tokens = (FeatureSequence) document.instance.getData();
            docNoise[doc] = new int[tokens.getLength()];
            docEmb[doc] = new int[tokens.getLength()];
            for (int position = 0; position < tokens.getLength(); position++) {
                int type = tokens.getIndexAtPosition(position);
                typeTotals[ type ]++;
            }
        ...
        for (TopicAssignment document : data) {
            ...
            FeatureSequence tokens = (FeatureSequence) document.instance.getData();
            FeatureSequence topicSequence =  (FeatureSequence) document.topicSequence;
            int docIndex = data.indexOf(document);
            ...
            for (int position = 0; position < tokens.size(); position++) {
                ...
                if (currentValue > maxTopicValue) { maxTopicValue = currentValue; }
                if (!noiseRoll(type, currentValue)) {
                    ...
                    docNoise[docIndex][position] = 0;
                } else {
                    noiseDistribution[type]++;
                    docNoise[docIndex][position] = 1;
                }
            }
        }
        logger.info("Max Noise Dist as of Initialization: " + Arrays.stream(noiseDistribution).max().getAsInt());
        logger.info("Max Topic Value as of Initialization: " + maxTopicValue);
        logger.info("Starting Embedding Space Size: " + closeWords.size());
    }
    ...
    public void estimate () throws IOException {
        ...
        if (numThreads > 1) {
            ...
            for (int thread = 0; thread < numThreads; thread++) {
                ...
                int[][] callableDocNoise = new int[data.size()][];
                for (int doc = 0; doc < data.size(); doc++) {
                    FeatureSequence fs = (FeatureSequence) data.get(doc).instance.getData();
                    int[] noise = new int[fs.getLength()];
                    System.arraycopy(docNoise[doc], 0, noise, 0, noise.length);
                    callableDocNoise[doc] = noise;
                }

                int[][] callableDocEmb = new int[data.size()][];
                for (int doc = 0; doc < data.size(); doc++) {
                    FeatureSequence fs = (FeatureSequence) data.get(doc).instance.getData();
                    int[] emb = new int[fs.getLength()];
                    System.arraycopy(docEmb[doc], 0, emb, 0, emb.length);
                    callableDocEmb[doc] = emb;
                }

                int[] callableNoiseDist = new int[numTypes];
                System.arraycopy(noiseDistribution, 0, callableNoiseDist, 0, numTypes);
                ...
                callables[thread] = new WorkerCallable(numTopics,
                                                       ...
                                                       alphabet,
                                                       offset, docsPerThread, skew, closeWords, optCloseWords,
                                                       callableNoiseDist, callableDocNoise, callableDocEmb);
            ...
            callables[0] = new WorkerCallable(numTopics,
                                              ...
                                              alphabet,
                                              offset, docsPerThread, skew, closeWords, optCloseWords,
                                              noiseDistribution, docNoise, docEmb);
            ...
            if (numThreads > 1) {
                ...
                for (int thread = 0; thread < numThreads; thread++) {
                    ...
                    for (int topic = 0; topic < numTopics; topic++) {
                        ...
                        totalTopicTokens1 += sourceTotals[topic];
                        assert (sourceTotals[topic] >= 0) : "Topic source below 0 " + sourceTotals[topic];
                    }
                }
                ...
                for (int thread = 0; thread < numThreads; thread++) {
                    // handle noiseDistribution
                    int[] sourceTotals = callables[thread].getNoiseDistribution();
                    for (int type = 0; type < numTypes; type++) {
                        noiseDistribution[type] += sourceTotals[type];
                        noiseTokens1 += sourceTotals[type];
//                         assert (sourceTotals[type] >= 0) : "Noise source below 0 " + sourceTotals[type];
                    }
                }

                for (int thread = 0; thread < numThreads; thread++) {
                    // Handle the docNoise array
                    int[][] docNoiseThread = callables[thread].getDocNoise();
                    for (int doc = callables[thread].startDoc;
                         doc < callables[thread].startDoc + callables[thread].numDocs;
                         doc++) {
                        int[] docNoiseOneDoc = docNoiseThread[doc];
                        docNoise[doc] = docNoiseOneDoc;
                    }
                }

                int totalTopicTokens2 = 0;
                int noiseTokens2 = 0;
                for (int doc = 0; doc < docNoise.length; doc++) {
                    int[] dn1 = docNoise[doc];
                    for (int position = 0; position < dn1.length; position++) {
                        totalTopicTokens2 += 1 - dn1[position];
                        noiseTokens2 += dn1[position];
                    }
                }
                
                for (int thread = 0; thread < numThreads; thread++) {
                    // Handle the docEmb array
                    int[][] docEmbThread = callables[thread].getDocEmb();
                    for (int doc = callables[thread].startDoc;
                         doc < callables[thread].startDoc + callables[thread].numDocs;
                         doc++) {
                        int[] docEmbOneDoc = docEmbThread[doc];
                        docEmb[doc] = docEmbOneDoc;
                    }
                }

                for (int doc = 0; doc < docEmb.length; doc++) {
                    int[] dn1 = docEmb[doc];
                    for (int position = 0; position < dn1.length; position++) {
                        totalTopicTokens2 += dn1[position];
                    }
                }

//                 assert (totalTopicTokens1 == totalTopicTokens2) : "topic tokens do not match " + totalTopicTokens1 + ", " + totalTopicTokens2;
//                 assert (noiseTokens1 == noiseTokens2) : "noise tokens do not match " + noiseTokens1 + ", " + noiseTokens2;
                ...
                // embedding space narrow down
                if (iteration >= burninPeriod && iteration % tau == 0) {
                    optCloseWords = new HashMap<String, List<String>>();
                    for (int type = 0; type < numTypes; type++) {
                        Object wordObj = alphabet.lookupObject(type);
                        String word = wordObj.toString();
                        if (closeWords.get(word) == null) { continue; }
                        int[] currentTypeTopicCounts = typeTopicCounts[type];
                        int noiseLevel = noiseDistribution[type];
                        int maxTopicCount = 0;
                        int totalFrequency = noiseLevel;
                        int index = 0;
                        while (index < currentTypeTopicCounts.length &&
				                currentTypeTopicCounts[index] > 0) {
				            int currentValue = currentTypeTopicCounts[index] >> topicBits;
				            totalFrequency += currentValue;
				            if (currentValue > maxTopicCount) { maxTopicCount = currentValue; }
				            index++;
                        }
                        double avgFrequency = totalFrequency / (numTopics+1);
                        double sum_of_squares = 0;
                        while (index < currentTypeTopicCounts.length &&
				                currentTypeTopicCounts[index] > 0) {
				            int currentValue = currentTypeTopicCounts[index] >> topicBits;
				            sum_of_squares += Math.pow((currentValue - avgFrequency), 2);
				            index++;
                        }
                        sum_of_squares += Math.pow((noiseLevel - avgFrequency), 2);
                        double std = Math.sqrt(sum_of_squares / numTopics);
                        if (noiseLevel > avgFrequency + (1.5*std) || maxTopicCount > avgFrequency + (1.5*std)) {
                            optCloseWords.put(word, closeWords.get(word));
                        }
                    }
                    logger.info("Total Size of Embedding Space: "+optCloseWords.size());
                }
                ...
                for (int thread = 0; thread < numThreads; thread++) {
                    copyCallables.add(new CopyCallable(callables[thread], typeTopicCounts, tokensPerTopic, optCloseWords));
                }
                try {
        ...
        while (iteration < iterations && totalChange > 0) {
            ...
            for (int doc = 0; doc < data.size(); doc++) {
                ...
                for (int position = 0; position < docLength; position++) {
                    ...
                    if (!noiseRoll(type, typeTopicCounts[type][newTopic])) {
    ...
    public void printTopNoise(File file, int numWords) throws IOException {
        PrintStream out = new PrintStream (file);
        Pair<String, Integer>[] pairs = getTopNoise();
        for (int index = 0; index < numWords; index++) {
            if (index < pairs.length) {
                Pair<String, Integer> pair = pairs[index];
                out.append(pair.k + ", " + pair.v + "\n");
            }
        }
        out.close();
    }
    ...
    public class Pair<X, Y> implements Comparable<Pair> {
        ...
        public int compareTo(Pair p2) {
            if (this.v < p2.v) { return 1; }
            else if (this.v > p2.v) { return -1; }
            else { return 0; }
        }
    }

    public Pair[] getTopNoise() {
        Pair[] pairs = new Pair[noiseDistribution.length];
        for (int index = 0; index < noiseDistribution.length; index++) {
            Object word = alphabet.lookupObject(index);
            pairs[index] = new Pair(word.toString(), noiseDistribution[index]);
        }
        Arrays.sort(pairs);
        return pairs;
    }
