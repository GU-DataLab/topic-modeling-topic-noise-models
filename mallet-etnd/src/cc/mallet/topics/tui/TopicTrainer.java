...
public class TopicTrainer {
    ...
    static CommandOption.String noiseFile = new CommandOption.String(TopicTrainer.class, "output-noise", "FILENAME", true, null,
         "The filename in which to write the noise distribution at the end of the iterations.  " +
         "By default this is null, indicating that no file will be written.", null);

    static CommandOption.Integer noiseWordsMax = new CommandOption.Integer(TopicTrainer.class, "noise-words-max", "INTEGER", true, 100,
         "The number of top noise words to write from the noise distribution at the end of the iterations.  " +
         "By default this is 100.", null);

    static CommandOption.String closeWordsFile = new CommandOption.String(TopicTrainer.class, "close-words-file", "FILENAME", true, null,
         "The filename where we find close word associations from embedding space.", null);
    ...
    static CommandOption.Integer skew = new CommandOption.Integer(TopicTrainer.class, "skew", "INTEGER", true, 0,
         "The skew of the beta distribution.", null);

    static CommandOption.Integer tau = new CommandOption.Integer(TopicTrainer.class, "tau", "INTEGER", true, 200,
         "The number of iterations to run between embedding space reoptimization.", null);
    ...
    public static void main (String[] args) throws java.io.IOException {
        ...
        if (inputModelFilename.value != null) {
            ...
            HashMap<String, List<String>> closeWords = new HashMap<String, List<String>>();
		    if (closeWordsFile != null) {
		        Scanner inCloseWords = new Scanner(new File(closeWordsFile.value));
		        while(inCloseWords.hasNext()) {
		            String line = inCloseWords.next();

		            List<String> cw = new ArrayList(Arrays.asList(line.split(",")));
		            if (cw.size() > 1) {
                        String key = cw.get(0);
                        cw = cw.subList(1, cw.size());
		                closeWords.put(key, cw);
		            }
		        }
		    }
            topicModel = new ParallelTopicModel (numTopics.value, alpha.value, beta.value, skew.value, tau.value, closeWords);
        }
...
//         }
        ...
        if (noiseFile.value != null) {
            topicModel.printTopNoise(new File (noiseFile.value), noiseWordsMax.value);
        }
