...
/**
 * Simple parallel threaded implementation of LDA (turned into TND),
 ...
 * edited for TND by Rob Churchill
 */
...
public class ParallelTopicModel implements Serializable {
    ...
    public ParallelTopicModel (int numberOfTopics) {
        this (numberOfTopics, numberOfTopics, DEFAULT_BETA, 0.0);
    }

    public ParallelTopicModel (int numberOfTopics, double skew)
	{
		this (numberOfTopics, 50.0, 0.01, skew);
	}

	public ParallelTopicModel (int numberOfTopics, double alphaSum, double beta, double skew) {
        this (newLabelAlphabet (numberOfTopics), alphaSum, beta, skew);
    }

    public ParallelTopicModel (int numberOfTopics, double alphaSum, double beta) {
        this (newLabelAlphabet (numberOfTopics), alphaSum, beta, 0.0);
    }
    ...
    public ParallelTopicModel (LabelAlphabet topicAlphabet, double alphaSum, double beta) {
        ...
        logger.info("Mallet NFT: " + numTopics + " topics, " + topicBits + " topic bits, " +
                    Integer.toBinaryString(topicMask) + " topic mask");
    }

    public ParallelTopicModel (LabelAlphabet topicAlphabet, double alphaSum, double beta, double skew) {
        ...
        if (numThreads > 1) {
            ...
            for (int thread = 0; thread < numThreads; thread++) {
                ...
                callables[thread] = new WorkerCallable(numTopics,
                                                       ...
                                                       offset, docsPerThread, skew,
                                                       callableNoiseDist, callableDocNoise);
            ...
            if (numThreads > 1) {
                ...
                for (int thread = 0; thread < numThreads; thread++) {
                    ...
                    for (int type = 0; type < numTypes; type++) {
                        ...
                        assert (sourceTotals[type] >= 0) : "source below 0 " + sourceTotals[type];
                    }
                }
